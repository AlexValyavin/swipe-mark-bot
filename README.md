...
4. **Находи нужное.** «Сохранёнки» с поиском, папками, тегами, фильтрами и группировкой по датам. Читай «Кратко», не открывая страницу.
...
- Карточки: превью, заголовок, источник, бейджи времени (`⏱ mm:ss` для видео, `📖 ~N мин` для чтения), «Кратко».
- Свайпы: **влево** — в архив, **вправо** — «позже» (карточка уходит из колоды в таб «Позже»), **вверх** — открыть.
- Прогресс разбора: счётчик «N осталось» + тонкая полоска прогресса над колодой.
- Экран завершения: итоги сессии («N → позже», «M → в архив») и кнопка «Посмотреть сохранёнки».
...
- Онбординг при первом входе: 3 шага и кнопка «Начать» (флаг `user_settings.onboarded`).
...
- **Продуктовая фаза P0** (по аудиту UX): свайп-семантика (право = позже), навигация «Разобрать · Сохранёнки · Позже», настройки в `⋯`, прогресс и completion screen, онбординг, «Кратко», BYOK в свёрнутую карточку — готово. Осталось: landing-страница (по желанию).
